...
type Result struct {
	...
	EnvPath        string // <Dir>/.env (contains ZTP_ADMIN_TOKEN=…)
...
func Scaffold(opts Options) (*Result, error) {
	...
	// Admin token — read existing if .env already had one, else mint.
	if existing := readAdminToken(res.EnvPath); existing != "" {
		res.AdminToken = existing
		track(res.EnvPath, false)
	} else {
		...
		if err := os.WriteFile(res.EnvPath, []byte(envFile(token)), 0o600); err != nil {
			return nil, fmt.Errorf("write .env: %w", err)
		}
		res.AdminToken = token
		track(res.EnvPath, true)
	}
...
// readAdminToken returns the ZTP_ADMIN_TOKEN value from a previously
// scaffolded .env file, or "" if the file is missing/unreadable/lacks
// the variable. Used to make Scaffold idempotent: a re-run on an
// existing tree returns the *same* token rather than minting a new
// one and breaking the running server.
func readAdminToken(envPath string) string {
	data, err := os.ReadFile(envPath)
	if err != nil {
		return ""
	}
	const key = "ZTP_ADMIN_TOKEN="
...
func envFile(token string) string {
	return "# Generated by ztp-server init. Do not commit this file.\n" +
...
# admin_token is supplied via the ZTP_ADMIN_TOKEN env var (see .env in
# this directory). The server refuses to start without one.
