...
func (s *Store) UpsertProfile(ctx context.Context, p store.ProfileRecord) error {
	if p.UpdatedAt.IsZero() {
		p.UpdatedAt = time.Now().UTC()
	}
	body := append([]byte(nil), p.BodyJSON...)
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO profiles (name, description, body, updated_at, updated_by)
		VALUES (?,?,?,?,?)
		...
		p.Name, nullStr(p.Description), body, p.UpdatedAt.UnixMilli(), nullStr(p.UpdatedBy))
	return err
}

func (s *Store) GetProfile(ctx context.Context, name string) (*store.ProfileRecord, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT name, description, body, updated_at, updated_by FROM profiles WHERE name = ?`, name)
	var p store.ProfileRecord
	var desc, by sql.NullString
	var body []byte
	var ts int64
	if err := row.Scan(&p.Name, &desc, &body, &ts, &by); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
		}
		return nil, err
	...
	p.UpdatedBy = by.String
	p.BodyJSON = append([]byte(nil), body...)
	p.UpdatedAt = time.UnixMilli(ts).UTC()
	return &p, nil
}

func (s *Store) ListProfiles(ctx context.Context) ([]store.ProfileRecord, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT name, description, body, updated_at, updated_by FROM profiles ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	out := make([]store.ProfileRecord, 0)
	for rows.Next() {
		var p store.ProfileRecord
		var desc, by sql.NullString
		var body []byte
		var ts int64
		if err := rows.Scan(&p.Name, &desc, &body, &ts, &by); err != nil {
			return nil, err
		}
		p.Description = desc.String
		p.UpdatedBy = by.String
